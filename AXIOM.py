...
name = []
for obj in X_prod1:
    if float(obj[2])==float(tahun):
        X_prod2.append(obj)

for obj in X_prod2:
    name.append(obj[0])
    prod.append(float(obj[3]))

cmap_name = 'tab20'
cmap = cm.get_cmap(cmap_name)
colors = cmap.colors[:len(name)]
fig, ax = plt.subplots()
fig.set_figwidth(36)
fig.set_figheight(9)
ax.bar(name, prod, color=colors)
ax.set_xticklabels(name, rotation=0,fontsize=10)
...
X_table["Produksi minyak mentah"] = prod
