...
st.caption("Line plot yang memproyeksikan produksi minyak mentah negara pilihan untuk keseluruhan tahun dengan sumbu-x menyatakan tahun produksi dan sumbu-y menyatakan besar produksi")
...
A_table["Produksi minyak mentah"] = A_graphy
...
try:
    ...
    for group in code:
        if group["name"]==negara:
            X_subreg = group["sub-region"]
    X_name = []
    X_prod2 = []
    for group in code:
        if group["sub-region"]==X_subreg and group["name"] in country_name:
            X_name.append(group["name"])
    print(X_subreg)
    ...
    for name in X_name:
        ...
        for set in data:
            if set[0]==name:
                X_prod1.append(float(set[3]))
        count = 0
        for year in year_list:
            if float(year)==float(tahun):
                break
            else:
                count = count + 1
        X_prod2.append(float(X_prod1[count]))

    cmap_name = 'tab20'
    cmap = cm.get_cmap(cmap_name)
    colors = cmap.colors[:len(X_name)]
    fig, ax = plt.subplots()
    fig.set_figwidth(36)
    fig.set_figheight(9)
    ax.bar(X_name, X_prod2, color=colors)
    ax.set_xticklabels(X_name, rotation=0,fontsize=10)
    ax.set_xlabel("Negara", fontsize=14)
    ax.set_ylabel("Produksi minyak mentah", fontsize=20)

    st.header("Bar Plot Produksi Minyak Mentah Sub-Region "+X_subreg+" pada Tahun "+tahun)
    st.caption("Bar plot yang menampilkan besar produksi minyak mentah negara-negara di sub-region negara pilihan pada tahun "+tahun)
    st.pyplot(fig)

    X_table = dict()
    X_code = []
    X_table["Negara"] = X_name
    for name in X_name:
        for group in code:
            if group["name"]==name:
                X_code.append(group["alpha-3"])
    X_table["Kode negara"] = X_code
    X_table["Produksi minyak mentah"] = X_prod2
    X_table = pd.DataFrame(X_table)

    with st.expander("Lihat Data Bar Plot"):
        st.subheader(" Tabel Data Produksi Minyak Mentah Sub-Region "+X_subreg+" pada Tahun "+tahun)
        st.table(X_table)
except:
    st.write("")
###EXTRAS###
...
with left_col.expander("Lihat Summary Data Tahun "+tahun):
    ...
    st.caption("Produksi terbesar menyatakan negara dengan produksi minyak mentah terbesar dan produksi terkecil menyatakan negara dengan produksi minyak mentah terkecil namun bukan nol")
    st.table(D_table1)
    st.subheader(" Tabel Zero Production Tahun "+str(tahun))
    st.caption("Kumpulan negara dengan besar produksi minyak mentah nol pada tahun "+tahun)
...
with right_col.expander("Lihat Summary Data Kumulatif"):
    ...
    st.caption("Kumpulan negara dengan besar produksi minyak mentah nol untuk keseluruhan tahun")
